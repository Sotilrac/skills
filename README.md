...
| [fdroid-publish](fdroid-publish/SKILL.md) | Getting an Android app into F-Droid and keeping it there: the fdroiddata recipe, the fastlane listing, reproducible builds, a tag-driven signed release pipeline, and the inclusion review checklist. Ships a pre-submission checker. |
| [html-deck](html-deck/SKILL.md) | Authoring slide decks as single-file [Bento](https://bento.page) presentations (`.bento.html`): the document is JSON inside the file, which carries its own viewer, editor, presenter and PDF export. Ships a CLI (fetch, extract, splice, embed fonts, headless check with per-slide screenshots), a generator template and the format guide. |
